...
putting a constraint on the internal points
...
class two_domain:
    ...
    def get_bl1(self,uG,u2): # Here the extra condition is imposed
        self.d1 = -1*np.dot(self.A1G.toarray(),np.transpose(uG))
        r2 = -1*(np.dot(self.AG2.toarray(),np.transpose(u2))-uG)
        return np.concatenate((self.d1,uG,r2))

    def get_bl2(self,u1,uG): # Here the extra condition is imposed
        r3 = (uG-np.dot(self.AG1.toarray(),np.transpose(u1)))
        self.d2 = -1*np.dot(self.A2G.toarray(),np.transpose(uG))
        return np.concatenate((r3,uG,self.d2))
    ...
    def initiate_constrained_matrices(self):
        ...
        C1 = scipy.sparse.csr_matrix(hstack((self.A11,zcolumn)))
        C2 = scipy.sparse.csr_matrix(hstack((zcolumn,self.A22)))
        # ================

        r11 = scipy.sparse.csr_matrix(hstack((np.transpose(self.A1)*self.A1,np.transpose(C1))))
        r12 = scipy.sparse.csr_matrix(hstack((C1,np.zeros(((self.n-2)**2,(self.n-2)**2)))))

        r21 = scipy.sparse.csr_matrix(hstack((np.transpose(self.A2)*self.A2,np.transpose(C2))))
        r22 = scipy.sparse.csr_matrix(hstack((C2,np.zeros(((self.n-2)**2,(self.n-2)**2)))))
