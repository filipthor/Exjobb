...
This class has in it two subdomains u1 and u2, imposing 
both neumann and dirichlet conditions along ur for both
domains.
...
class two_domain:
    ...
    def get_A1(self):
        ...
        r2 = scipy.sparse.csr_matrix(hstack((self.AG1,self.AGG2)))
        return scipy.sparse.csr_matrix(vstack((r1,r2)))

    def get_A2(self):
        zrow = np.zeros((self.n-2, (self.n-2)**2))
        zcolumn = np.transpose(zrow)
        r3 = scipy.sparse.csr_matrix(hstack((self.AGG2,self.AG2)))
        r4 = scipy.sparse.csr_matrix(hstack((zcolumn,self.A22)))
        return scipy.sparse.csr_matrix(vstack((r3,r4)))
